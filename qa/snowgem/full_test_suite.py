...
#!/usr/bin/env python2
#
# Execute all of the automated tests related to Vidulum.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/vidulum/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/vidulum/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/vidulumd')
    ret &= test_rpath_runpath('src/vidulum-cli')
    ret &= test_rpath_runpath('src/vidulum-gtest')
    ret &= test_rpath_runpath('src/vidulum-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/vidulum/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/vidulumd')
    ret &= test_fortify_source('src/vidulum-cli')
    ret &= test_fortify_source('src/vidulum-gtest')
    ret &= test_fortify_source('src/vidulum-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/vidulum/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/vidulum-gtest')],
