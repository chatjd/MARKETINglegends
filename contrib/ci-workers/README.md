...
installation for use as a Buildbot worker in Vidulum's CI.
...
- Add a regular (non-latent) worker to the master.cfg for dev-ci.vidulum.org, and
  deploy the changes.
...
- Start a basic EC2 instance using the template AMI for the target OS.
  - Choose the smallest instance size, it won't be used for building Vidulum.
