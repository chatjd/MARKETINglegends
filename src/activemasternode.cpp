...
//
// Bootup the Masternode, look for a Vidulum input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if(NetworkIdFromCommandLine() == CBaseChainParams::MAIN) {
            if(service.GetPort() != 7676) {
                notCapableReason = strprintf("Invalid port: %u - only 7676 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if(service.GetPort() == 7676) {
            notCapableReason = strprintf("Invalid port: %u - 7676 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if(NetworkIdFromCommandLine() == CBaseChainParams::MAIN) {
        if (service.GetPort() != 7676) {
            errorMessage = strprintf("Invalid port %u for masternode %s - only 7676 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == 7676) {
        errorMessage = strprintf("Invalid port %u for masternode %s - 7676 is only supported on mainnet.", service.GetPort(), strService);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
