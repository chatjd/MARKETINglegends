...
int printMiningStatus(bool mining)
{
    ...
    if (mining) {
        ...
        std::cout << _("To enable mining, add 'gen=1' to your vidulum.conf and restart.") << std::endl;
        lines += 2;
    }
    std::cout << std::endl;
        ...
        CAmount mature {0};
        {
            ...
            while (it != u->end()) {
                ...
                if (mapBlockIndex.count(hash) > 0 &&
                    ...
                    if ((height > 0) && (height < consensusParams.GetLastVRewardSystemBlockHeight())
                        && (height >= consensusParams.vUpgrades[Consensus::UPGRADE_OVERWINTER].nActivationHeight)) {

                        subsidy -= subsidy * 20 / 100;
                    }
...
void ThreadShowMetricsScreen()
{
    ...
    RenameThread("vidulum-metrics-screen");
    ...
    if (isScreen) {
        ...
        // Thank you text
        std::cout << _("Thank you for running a Vidulum node!") << std::endl;
