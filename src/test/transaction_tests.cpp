...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    {
        JSDescription jsdesc(false, *pvidulumParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pvidulumParams, verifier, joinSplitPubKey));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pvidulumParams, verifier, joinSplitPubKey));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(false, *pvidulumParams, joinSplitPubKey, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(false, *pvidulumParams, joinSplitPubKey, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(false, *pvidulumParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pvidulumParams, verifier, joinSplitPubKey));
    }
}
