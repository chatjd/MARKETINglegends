...
CAmount CWalletTx::GetDenominatedCredit(bool unconfirmed, bool fUseCache) const
{
    ...
    const int64_t nBlockTime = (STANDARD_LOCKTIME_VERIFY_FLAGS & LOCKTIME_MEDIAN_TIME_PAST)
                             ? chainActive.Tip()->GetMedianTimePast()
                             : GetAdjustedTime();

    bool isUnconfirmed = !IsFinalTx(*this, nBlockHeight, nBlockTime) || (!IsTrusted() && nDepth == 0);
...
CAmount CWalletTx::GetAnonymizableCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        const int rounds = pwallet->GetInputObfuscationRounds(vin);
        if (rounds >= -2 && rounds < nVidulumSendRounds) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputObfuscationRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nVidulumSendRounds;
            }
        }
    }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (IsLockedCoin((*it).first, i) && coin_type != ONLY_15000)
                {
                    continue;
                }
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet,  bool& fOnlyCoinbaseCoinsRet, bool& fNeedCoinbaseCoinsRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if (coin_type == ONLY_DENOMINATED) {
                ...
                if (rounds < nVidulumSendRounds) continue;
            }
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 VDL
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 VDL over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        //if(out.tx->vout[out.i].nValue == 15000*COIN) continue;
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            bool fAccepted = false;

            // Function returns as follows:
            //
            // bit 0 - 10000 VDL+1 ( bit on if present )
            // bit 1 - 1000 VDL+1
            // bit 2 - 100 VDL+1
            // bit 3 - 10 VDL+1
            // bit 4 - 1 VDL+1
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, fOnlyCoinbaseCoins, fNeedCoinbaseCoins, coinControl, coin_type, useIX))
                {
                    ...
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason += " " + _("Unable to locate enough funds for this transaction that are not equal 15000 VDL.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT15000IFMN) {
                        strFailReason += " " + _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 15000 VDL.");
                    } else {
...
void CWallet::GetAllReserveKeys(set<CKeyID>& setAddress) const
{
    ...
    BOOST_FOREACH(const int64_t& id, setKeyPool)
    {
        ...
        if (!HaveKey(keyID))
            throw runtime_error("GetAllReserveKeyHashes(): unknow