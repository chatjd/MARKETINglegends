...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (NetworkIdFromCommandLine() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 7676) return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
...

    {
        ...
        if (!AcceptableInputs(mempool, state, CTransaction(tx), false, NULL)) {
            LogPrint("masternode", "!AcceptableInputs\n");
            //set nDos
    ...
    // should be at least not earlier than block when 15000 VDL tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = uint256();
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 Vidulum tx -> 1 confirmation
