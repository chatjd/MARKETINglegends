...
ZCJoinSplit* pvidulumParams = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "vidulum.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "vidulumd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7676, 7677));
    ...
    strUsage += HelpMessageOpt("-enablevidulumsend=<n>",strprintf(_("Enable use of automated darksend for funds stored in this wallet (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-vidulumsendrounds=<n>",strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2));
    strUsage += HelpMessageOpt("-anonymizevidulumamount=<n>",strprintf(_("Keep N VIDULUM anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Vidulum specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 17676, 27676));
...
)
{
    ...
    if (!(
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Vidulum network parameters in the following directory:\n"
              "%s\n"
              "Please run 'vidulum-fetch-params' or './zcutil/fetch-params.sh' and then restart."),
                ZC_GetParamsDir()),
    ...
    pvidulumParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Vidulum is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vidulum is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vidulum is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize Vidulum circuit parameters
    ...
    while (!fLoaded) {
        ...
        do {
            try {
				...
				// Vidulum: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Vidulum") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Vidulum to complete") << "\n";
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Vidulum was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Vidulum with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Vidulum was not built with wallet support. Set -mineraddress, or rebuild Vidulum with wallet support."));
    }
 #endif // !ENABLE_WALLET
    ...
    fEnableVidulumSend = GetBoolArg("-enablevidulumsend", false);

    nVidulumSendRounds = GetArg("-vidulumsendrounds", 10);
    if (nVidulumSendRounds > 100) nVidulumSendRounds = 100;
    if (nVidulumSendRounds < 10) nVidulumSendRounds = 10;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if(nLiquidityProvider != 0) {
        obfuScationPool.SetMinBlockSpacing(std::min(nLiquidityProvider,100)*15);
        fEnableVidulumSend = true;
        nVidulumSendRounds = 99999;
    }

    nAnonymizeVidulumAmount = GetArg("-anonymizevidulumamount", 0);
    if (nAnonymizeVidulumAmount > 999999) nAnonymizeVidulumAmount = 999999;
    if (nAnonymizeVidulumAmount < 2) nAnonymizeVidulumAmount = 2;
    ...
    LogPrintf("Vidulum send rounds %d\n", nVidulumSendRounds);
    LogPrintf("Anonymize Vidulum Amount %d\n", nAnonymizeVidulumAmount);
    /* Denominations
