...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"vidulumaddress\"          (string) Vidulum address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue verifytxoutproof(const UniValue& params, bool fHelp)
{
    ...
    if (merkleBlock.txn.ExtractMatches(vMatch, chainActive.Tip()->nHeight + 1) != merkleBlock.header.hashMerkleRoot)
        return res;
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the vidulum address, the value is the " + CURRENCY_UNIT + " amount\n"
            "      ,...\n"
            "    }\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"t12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) vidulum address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
