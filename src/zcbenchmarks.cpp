...
double benchmark_create_joinsplit()
{
    ...
    assert(jsdesc.Verify(*pvidulumParams, verifier, joinSplitPubKey));
    return ret;
}
...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 joinSplitPubKey;
    auto verifier = libzcash::ProofVerifier::Strict();
    joinsplit.Verify(*pvidulumParams, verifier, joinSplitPubKey);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    auto sk = libzcash::SproutSpendingKey::random();
    auto tx = GetValidReceive(*pvidulumParams, sk, 10, true);
...
double benchmark_increment_note_witnesses(size_t nTxs)
{
    ...
    for (int i = 0; i < nTxs; i++) {
        auto wtx = GetValidReceive(*pvidulumParams, sk, 10, true);
        auto note = GetNote(*pvidulumParams, sk, wtx, 0, 1);
