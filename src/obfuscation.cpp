    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Vidulum and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 VDL per transaction on average.
            */
            if (r <= 10) {
...
{
    if (!fEnableVidulumSend && !fMasterNode) return;
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (!fEnableVidulumSend) return false;
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) return false;
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeVidulumAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nVidulumSendRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nVidulumSendRounds)) {
...
bool CObfuscationPool::PrepareObfuscationDenominate()
{
    ...
    for (int i = 0; i < nVidulumSendRounds; i++) {
    ...
    strError = pwalletMain->PrepareObfuscationDenominate(0, nVidulumSendRounds);
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
        nFeeRet, nChangePos, strFail, &coinControl, ONLY_NONDENOMINATED_NOT15000IFMN);
    if (!success) {
        // if we failed (most likeky not enough funds), try to use all coins instead -
        // MN-like funds should not be touched in any case and we can't mix denominated without collaterals anyway
        nChangePos = -1;
        CCoinControl* coinControlNull = NULL;
        LogPrintf("MakeCollateralAmounts: ONLY_NONDENOMINATED_NOT15000IFMN Error - %s\n", strFail);
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, nChangePos, strFail, coinControlNull, ONLY_NOT15000IFMN);
        if (!success) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100XLR+1 ( bit on if present )
    // bit 1 - 10XLR+1
    // bit 2 - 1XLR+1
    // bit 3 - .1XLR+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("vidulum-obfuscation");
