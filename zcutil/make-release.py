...
def main(args=sys.argv[1:]):
    """
    Perform the final Vidulum release process up to the git tag.
    """
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "vidulum-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "vidulum-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './vidulum-make-release.log'
    ...
    logging.info('vidulum make-release.py debug log: %r', logname)
