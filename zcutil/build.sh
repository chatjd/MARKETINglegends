...
$0 [ --enable-lcov || --disable-tests ] [ --disable-mining ] [ --disable-rust ] [ --enable-proton ] [ --disable-libs ] [ MAKEARGS... ]
  ...
  source. MAKEARGS are applied to both dependencies and Vidulum itself.

  If --enable-lcov is passed, Vidulum is configured to add coverage
  instrumentation, thus enabling "make cov" to work.
  If --disable-tests is passed instead, the Vidulum tests are not built.

  If --disable-mining is passed, Vidulum is configured to not build any mining
  code. It must be passed after the test arguments, if present.

  If --disable-rust is passed, Vidulum is configured to not build any Rust language
  assets. It must be passed after test/mining arguments, if present.

  If --enable-proton is passed, Vidulum is configured to build the Apache Qpid Proton
