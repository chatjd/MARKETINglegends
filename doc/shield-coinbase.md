...
The current Vidulum protocol includes a consensus rule that coinbase rewards must be sent to a shielded address.
...
The default fee is 0.0010000 VDL and the default limit on the maximum number of UTXOs to shield is 50.

## Examples

Sweep up coinbase UTXOs from a transparent address you use for mining:

    vidulum-cli z_shieldcoinbase tMyMiningAddress zMyPrivateAddress

Sweep up coinbase UTXOs from multiple transparent addresses to a shielded address:

    vidulum-cli z_shieldcoinbase "*" zMyPrivateAddress

Sweep up with a fee of 1.23 VDL:

    vidulum-cli z_shieldcoinbase tMyMiningAddress zMyPrivateAddress 1.23

Sweep up with a fee of 0.1 VDL and set limit on the maximum number of UTXOs to shield at 25:

    vidulum-cli z_shieldcoinbase "*" zMyPrivateAddress 0.1 25
...
If the limit parameter is set to zero, the vidulumd `mempooltxinputlimit` option will be used instead, where the default value for `mempooltxinputlimit` is zero, which means no limit.
...
- Block 141042 is almost ~2 MB in size (the maximum size for a block) and contains 1 coinbase reward transaction and 20 transactions, each indivually created by a call to z_shieldcoinbase.
  - https://explorer.testnet.vidulum.org/block/0050552a78e97c89f666713c8448d49ad1d7263274422272696187dedf6c0d03
- Drilling down into a transaction, you can see there is one joinsplit, with 244 inputs (vin) and 0 outputs (vout).
  - https://explorer.testnet.vidulum.org/tx/cf4f3da2e434f68b6e361303403344e22a9ff9a8fda9abc180d9520d0ca6527d
